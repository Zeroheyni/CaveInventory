...
export async function syncCharacter(characterId: string) {
  ...
  const { data: character, error } = await client.from('characters').select('id, max_carga, data').eq('id', characterId).single();
  ...
  const invText = buildCharacterInventoryText(character.max_carga, data);
